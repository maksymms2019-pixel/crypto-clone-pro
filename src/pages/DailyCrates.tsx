...
const tierStyles = {
  common: { icon: Coins, className: "border-[#e7b650]/30 bg-[#e7b650]/10 text-[#f4d58a]" },
  silver: { icon: Coins, className: "border-[#b9cdeb]/30 bg-[#b9cdeb]/10 text-[#dbe8fa]" },
  diamond: { icon: Gem, className: "border-[#7cd4f5]/40 bg-[#7cd4f5]/10 text-[#a9ebff]" },
} as const;
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      <section className="relative overflow-hidden rounded-[28px] border border-[var(--gold)]/25 bg-[radial-gradient(circle_at_50%_20%,rgba(231,182,80,.18),transparent_48%),linear-gradient(145deg,#102b35,#07141b)] p-6 text-center shadow-[0_20px_70px_-30px_rgba(231,182,80,.45)]">
        ...
        <div className="relative mx-auto mb-5 flex h-28 w-28 items-center justify-center rounded-[30px] border border-[#f4d58a]/40 bg-gradient-to-br from-[#ffe9a3] via-[#e7b650] to-[#9c6527] shadow-[0_14px_45px_-8px_rgba(231,182,80,.75)] transition-transform duration-500 hover:rotate-3 hover:scale-105"><Box size={58} strokeWidth={1.35} className="text-[#241506]" /><Sparkles size={20} className="absolute right-3 top-3 text-[#fff4c9]" /></div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-[.22em] text-[var(--gold-soft)]">Скриня дня</p><h2 className="display text-2xl font-bold text-[var(--text)]">Відкрий свій лут</h2><p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-[var(--text-muted)]">5 випадкових монет із шансом вибити срібну або діамантову.</p>
        {!user ? <Link to="/auth" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[var(--gold)] px-5 py-3 text-sm font-bold text-[#1a0f00] transition-transform hover:scale-[1.02]"><LockKeyhole size={16} /> Увійти, щоб відкрити</Link> : <button type="button" onClick={openCrate} disabled={opening || Boolean(result)} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[var(--gold)] px-6 py-3 text-sm font-bold text-[#1a0f00] transition-all hover:scale-[1.02] hover:shadow-[0_0_28px_rgba(231,182,80,.35)] disabled:cursor-not-allowed disabled:opacity-60">{opening ? <><Sparkles size={16} className="animate-spin" /> Відкриваємо...</> : result ? <><Check size={16} /> Уже відкрито сьогодні</> : <><Box size={16} /> Відкрити скриню</>}</button>}
      </section>
      <div className="grid grid-cols-3 gap-2 text-center text-xs">{[{ label: "Монетка", value: "75%" }, { label: "Срібна", value: "20%" }, { label: "Діамант", value: "5%" }].map((item) => <div key={item.label} className="rounded-2xl border border-[var(--line)] bg-[var(--bg-elev)] px-2 py-3"><div className="font-bold text-[var(--text)]">{item.value}</div><div className="mt-1 text-[var(--text-muted)]">{item.label}</div></div>)}</div>
